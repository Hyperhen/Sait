...
const ClubPhoto = require("../models/ClubPhoto");
...
function readPhotosFs() {
  try {
    const arr = JSON.parse(fs.readFileSync(PHOTOS_FILE, "utf8"));
    return Array.isArray(arr) ? arr : [];
  } catch {
    return [];
  }
...
function normalizePhotosFs() {
  let list = readPhotosFs();
  let changed = false;
  list = list
    .filter((row) => row && typeof row.title === "string" && typeof row.filename === "string")
    .map((row) => {
      if (!row.id) changed = true;
      return {
        ...row,
...
async function getPhotosWithIds() {
  if (useMongo()) {
    await mongoReady();
    const docs = await ClubPhoto.find().sort({ uploadedAt: -1 }).lean();
    return docs.map((d) => ({
      id: String(d._id),
      title: d.title,
  ...
  return normalizePhotosFs();
}

async function getPhotosPublic() {
  const photos = await getPhotosWithIds();
  return photos.map(({ title, description, imageUrl, uploadedAt }) => ({
    title,
    description,
    imageUrl,
    uploadedAt,
...
async function addPhoto(title, description, filename) {
  const imageUrl = `/uploads/${filename}`;
  
  if (useMongo()) {
    await mongoReady();
    await ClubPhoto.create({
      title,
      description,
      imageUrl,
    });
...
async function deletePhotoById(rawId) {
  const id = parseEntityId(rawId);
  if (!id) return { ok: false, reason: "bad_id" };
  ...
  if (useMongo()) {
    if (!MONGO_ID_HEX_RE.test(id)) return { ok: false, reason: "bad_id" };
    await mongoReady();
    const photo = await ClubPhoto.findById(id);
    ...
    await ClubPhoto.deleteOne({ _id: id });
    return { ok: true };
  }
  ...
  const list = normalizePhotosFs();
  const photo = list.find((row) => row.id === id);
  if (!photo) return { ok: false, reason: "not_found" };
